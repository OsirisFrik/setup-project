...
import { buildInstallCommand } from "../package-manager/detector.js";
import { loadTemplate, processTemplate } from "./template-processor.js";
            ...
            if (step.dependsOn) {
                for (const dependency of step.dependsOn) {
                    if (!completedSteps.has(dependency)) {
                        throw new Error(`Dependency '${dependency}' not completed before step '${step.id}'`);
                    }
                }
            }
...
function executeInstallDeps(step, context) {
...
function executeRunCommand(step, context) {
    const config = step.config;
...
function executeGenerateFromTemplate(step, context) {
    const config = step.config;
    const outputs = [];
    for (const file of config.files) {
        const templatePath = join(context.projectRoot, file.template);
...
function executeCopyFile(step, context) {
    const config = step.config;
