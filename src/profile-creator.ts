...
import { getProfilePath, getProfilesDir, ensureProfilesDir } from './utils/paths.ts';
import { loadProfile } from './profiles/loader.ts';
import type { Profile, Dependencies, Step } from './types.ts';
...
export async function createProfileInteractive(): Promise<void> {
  console.log('\n📋 Create New Profile\n');
...
export async function deleteProfile(profileId: string): Promise<void> {
  ...
  const confirm = await prompt(
    `⚠ Are you sure you want to delete '${profileId}'? (y/N): `
  );
